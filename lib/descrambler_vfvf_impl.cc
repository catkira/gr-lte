...
namespace gr {
  namespace lte {
    ...
    {
      return gnuradio::make_block_sptr<descrambler_vfvf_impl>(
        tag_key, msg_buf_name, len, name);
    }
     ...
     */
    descrambler_vfvf_impl::descrambler_vfvf_impl(std::string tag_key, std::string msg_buf_name, int len, std::string name)
      : gr::sync_block("descrambler_vfvf",
              gr::io_signature::make(1 /* min inputs */, 1 /* max inputs */, sizeof(input_type) * len),
              gr::io_signature::make(1 /* min outputs */, 1 /*max outputs */, sizeof(output_type) * len)),
              d_len(len),
              d_seq_index(0),
              d_part(0)              
    {
		...
		set_msg_handler(d_msg_buf, [this](const pmt::pmt_t& msg){ handle_msg(msg);});        
    }
