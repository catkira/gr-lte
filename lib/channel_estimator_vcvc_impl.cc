...
namespace gr {
  namespace lte {
    ...
    channel_estimator_vcvc::make(int rxant, int subcarriers, std::string tag_key, std::string msg_buf_name, const std::vector<std::vector<int> > &pilot_carriers, const std::vector<std::vector<gr_complex> > &pilot_symbols, std::string name)
    {
      return gnuradio::make_block_sptr<channel_estimator_vcvc_impl>(
        rxant, subcarriers, tag_key, msg_buf_name, pilot_carriers, pilot_symbols, name);
    }
                ...
                d_rxant(rxant)
    {
      ...
      set_msg_handler(
          d_msg_buf,
          [this](const pmt::pmt_t& msg){ handle_msg(msg);});
