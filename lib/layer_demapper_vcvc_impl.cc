...
namespace gr {
  namespace lte {
    ...
    {
      return gnuradio::make_block_sptr<layer_demapper_vcvc_impl>(
        N_ant, vlen, style, name);
    }
     ...
     */
    layer_demapper_vcvc_impl::layer_demapper_vcvc_impl(int N_ant, int vlen, std::string style, std::string name)
			  ...
			  d_style("null")              
    {
		...
		set_msg_handler(pmt::mp("N_ant"), [this](const pmt::pmt_t& msg){ handle_msg(msg);});        
    }
